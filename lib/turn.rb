...
class Turn
    ...
    def computer_shot
        ...
        if array_of_hits != [] 
            ...
            array_of_hits.each do |cell|
                ...
                array_of_smart_guess_options.each do |coord|
                    if player_board.cells.key?(coord) == true and coord.fired_upon? == false
            ...
            p "line 89 #{computer_smart_guess}"
            computer_smart_guess = player_board.cells[computer_smart_guess]
            p "line 91 #{computer_smart_guess}"

            computer_smart_guess
        else
            computer_smart_guess = array_of_board_coordinates.sample(1)
            p "line 96 #{computer_smart_guess}"
            computer_smart_guess = player_board.cells[computer_smart_guess[0]]
            p "line 98 #{computer_smart_guess}"
            computer_smart_guess
        end
        ...
        if computer_smart_guess.render(true) == "H"
            ...
            turn_result = "My shot on #{computer_smart_guess.coordinate} was a hit! I sunk your #{coord.ship[0].name}!"
        end
